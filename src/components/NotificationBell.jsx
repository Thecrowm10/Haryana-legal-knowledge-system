...
const TYPE_META = {
  new_upload:         { icon: FileText,     color: '#0d6efd', bg: 'rgba(13, 110, 253,.1)',  label: 'New Upload' },
  document_approved:  { icon: CheckCircle,  color: '#198754', bg: 'rgba(25, 135, 84,.1)',   label: 'Approved' },
  document_rejected:  { icon: XCircle,      color: '#dc3545', bg: 'rgba(220, 53, 69,.1)',   label: 'Rejected' },
};
        ...
        <div className="notif-panel" style={{
          ...
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--surface-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-heading)' }}>
              Notifications {unread > 0 && <span style={{ fontSize: 11, fontWeight: 600, color: '#dc3545', marginLeft: 4 }}>({unread} new)</span>}
            </div>
            {unread > 0 && (
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: n.read ? 'transparent' : 'rgba(13, 110, 253,.04)',
                      cursor: 'pointer', transition: 'background .12s',
                    }}
                    onMouseEnter={e => e.currentTarget.style.background = 'var(--surface-hover)'}
                    onMouseLeave={e => e.currentTarget.style.background = n.read ? 'transparent' : 'rgba(13, 110, 253,.04)'}
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                        ...
                        {!n.read && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#0d6efd', flexShrink: 0 }} />}
                      </div>
                      ...
                      {n.remark && (
                        <div style={{
                          ...
                          background: n.type === 'document_rejected' ? 'rgba(220, 53, 69,.07)' : 'rgba(25, 135, 84,.07)',
                          border: `1px solid ${n.type === 'document_rejected' ? 'rgba(220, 53, 69,.2)' : 'rgba(25, 135, 84,.2)'}`,
                          borderLeft: `3px solid ${n.type === 'document_rejected' ? '#dc3545' : '#198754'}`,
                          borderRadius: 6,
                        }}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: n.type === 'document_rejected' ? '#dc3545' : '#16a34a', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '.05em' }}>
